...
- **BUG-Books-004 Layer A — connector treats any HTTP 200 from Tally as success without parsing the response body.** `connector/tally_client.post_voucher` returned `status:"success"` regardless of Tally's `<CREATED>0</CREATED>` / `<EXCEPTIONS>1</EXCEPTIONS>` structured rejection — backend wrote `voucher.posted_to_tally` for vouchers Tally explicitly rejected. Silent books-vs-Tally drift with no error surface. **Code fix shipped 2026-05-21 in commit `4832688`** (also closes Layer B): connector now parses the ImportData response envelope and raises `TallyImportRejected` (strict rejection) or `TallyAmbiguousResponse` (shape drift); backend dispatcher reads `envelope.retryable` and raises `TallyRetryableEnvelope` / `TallyRejectedEnvelope` instead of returning. **§7.5b rejection-lane VALIDATED PASS 2026-05-22.** Live POST against rebuilt connector + TallyPrime with Taxmind Books loaded: voucher `a860acf0-1772-48b6-b731-8105710db637` (Sales 100) was rejected by Tally with `<LINEERROR>Ledger 'Sales' does not exist!</LINEERROR>`. The connector parsed the envelope, the dispatcher emitted `voucher.tally_post_failed` with `error.code=TallyImportRejected`, and **no phantom voucher landed in Tally** (user verified the Day Book directly). The wire/audit contract works end-to-end on a live system. §7.5b *happy-path* PASS is the remaining live check; its blocker BUG-Books-005 (below — every locally-known ledger had `tally_master_id=NULL`) **shipped its fix 2026-05-25 (`ec68199`), and live validation is now deferred operational verification, not a Phase 0 gate.** **Layer C (durable `tally_voucher_guid` / REMOTEID survivability) is also deferred** — it waited on a real Tally success envelope, itself gated on §7.5b happy-path PASS, so it moves to the first real-user successful POST. The candidate root causes listed in the original BUG-004 filing — fiscal year, VCHTYPE case, GST drift, party-ledger context — are ruled out. Reproducers `5b7e2573-…` (Tally up no company loaded, 2026-05-19), `7b1e4328-…` (documented happy path, 2026-05-20), and `a860acf0-…` (rejection-lane PASS evidence + BUG-005 reproducer + BUG-002 rejection-class recurrence, 2026-05-22) preserved as evidence — do not cancel or repost. Cluster context: "Validation findings during closeout" §"Update 2026-05-19/20" above.
- **BUG-Books-005 — local ledgers never synced to Tally master list; voucher post guard. SHIPPED 2026-05-25; live validation DEFERRED.** Surfaced 2026-05-22 by the BUG-004 Layer A validation cycle: company `58f01ad7-…`'s six ledger rows (including the five §7.5 sync_masters claimed to have synced on 2026-05-16) all carried `tally_master_id=NULL`, so connector voucher XML named ledgers Tally couldn't resolve and every POST was rejected with `<LINEERROR>Ledger '<name>' does not exist!</LINEERROR>`. **Implementation SHIPPED 2026-05-25 (commit `ec68199`, CI green)** via the PULL direction (Tally GUID as the durable identifier, per the external-review decision): connector requests + parses each ledger's `<GUID>` and emits it in the `sync_masters` payload (steps 2a–2c, `57b2ed9`/`d87f87c`/`abd08ae`); backend `upsert_from_sync` writes `tally_master_id`, applies a four-arm NULL/GUID reconcile matrix, and stamps `tally_synced_at` (steps 2d–2e, `6e69f03`/`38e219e`); a two-layer voucher post guard (`check_ledgers_synced`, fresh query every call) rejects any voucher referencing an unsynced ledger — API layer returns 422 `ledger_not_synced_to_tally` with operator-readable remediation (no row created), dispatcher layer is defense-in-depth emitting a `voucher.tally_post_blocked` audit (step 3, `ec68199`). **Automated validation complete** — backend 565/565, +13 sync-ingest tests (2d–2e), +6 voucher-guard tests (step 3), plus connector-side GUID parse/emit tests (2a–2c); CI green run 26386836965. **Live Tally validation: DEFERRED operational verification — NOT a Phase 0 gate.** Three sessions of attempted live validation (2026-05-24/25/26) consistently reached environment setup but not the validation moment, due to an unstable local integration environment (Docker cold-start, Tally ODBC re-enable, connector re-enrollment + stale-`.exe`/rebuild fragility, 30-minute access-token churn, backend process instability). The marginal value of further local validation loops collapsed relative to engineering time; **the first real-user encounter becomes the practical operational validation point.** Risk is bounded: the post guard fails safe (422 with a concrete `/connector/sync/{company_id}` remediation pointer), and auditability + reconciliation semantics are covered by automated tests. Operational lessons captured in the BUG-005 memory entry. Reproducer: voucher `a860acf0-…` (same row as the BUG-004 Layer A PASS evidence) — preserved, do not cancel. Cross-link: BUG-004 Layer C (REMOTEID / `tally_voucher_guid` survivability) is similarly deferred to the first real-user successful POST. Cluster context: "Validation findings during closeout" §"Update 2026-05-19/20" above.
