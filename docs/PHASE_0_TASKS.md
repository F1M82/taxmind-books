...
- **AuditContext source tagging on dispatch failures.** Eager-mode failures currently tag the resulting audit row as `source=worker` via `dispatch_voucher_to_tally`'s existing `AuditContext`. Mechanically incorrect — in eager mode the dispatch runs inside the API uvicorn process, not a Celery worker. Cosmetic for now (the audit chain is still correct on `entity_id` / `action` / timestamps; only the `source` field misrepresents the originating process). Revisit when P0.58 option 2 (Redis pub/sub) lands, so the `source` field actually distinguishes which process attempted the dispatch — and so the worker source isn't used as a load-bearing diagnostic when both modes coexist.
