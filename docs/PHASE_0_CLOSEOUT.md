...
- **BUG-Books-004 Layer A — connector treats any HTTP 200 from Tally as success without parsing the response body.** `connector/tally_client.post_voucher` returned `status:"success"` regardless of Tally's `<CREATED>0</CREATED>` / `<EXCEPTIONS>1</EXCEPTIONS>` structured rejection — backend wrote `voucher.posted_to_tally` for vouchers Tally explicitly rejected. Silent books-vs-Tally drift with no error surface. **Code fix shipped 2026-05-21 in commit `4832688`** (also closes Layer B): connector now parses the ImportData response envelope and raises `TallyImportRejected` (strict rejection) or `TallyAmbiguousResponse` (shape drift); backend dispatcher reads `envelope.retryable` and raises `TallyRetryableEnvelope` / `TallyRejectedEnvelope` instead of returning. **§7.5b validation pass still owed** before Phase 1 ship-gate clears — requires PyInstaller rebuild + AV re-clearance + connector re-enrollment + fresh live-Tally run with deliberate failure injection. Reproducers `5b7e2573-…` (Tally up with no company loaded, 2026-05-19) and `7b1e4328-…` (Tally up with Taxmind Books loaded on the documented happy path, 2026-05-20) preserved as evidence — do not cancel or repost. The masked secondary bug that voucher `7b1e4328-…` hit (Tally rejecting the happy-path voucher for some still-unknown reason) becomes visible after the validation pass via the new `TallyImportRejected.line_error` field. Cluster context: "Validation findings during closeout" §"Update 2026-05-19/20" above.
