...
| `voucher.tally_post_retry_requested` | (BUG-002) Operator explicitly re-triggered a Tally post for a stranded `pending_tally_post` voucher (typically after fixing a `tally_post_failed` rejection or a `tally_post_blocked` unsynced-ledger). `source='api'`; `new_value` carries `requested_by` and the `prior_error`. The subsequent dispatch emits its own outcome row (`posted_to_tally` / `tally_post_failed` / `tally_post_blocked` / `tally_post_queued`). This is the manual counterpart to the automatic retryable-class re-enqueue, which only covers `tally_post_queued` strands. |
